#!/usr/bin/env bash
# verify.sh — 10-gate verification flow for mcp-formal.
#
...
for arg in "$@"; do
  case "$arg" in
    ...
    -h|--help)
      cat <<'EOF'
verify.sh — mcp-formal verification suite
  --gate N       Run only the given gate (1..6)
                   1  spec lock & package integrity
                   2  static typecheck & build
                   3  unit tests (parsers & contract)
                   4  fixture integration (podman simulation/lint)
                   5  stdio protocol contract (json-rpc ping)
                   6  docs verification
EOF
      exit 0
      ;;
...
run_gate_6() {
  ...
  grep -q "formal_prove" README.md || fail 6 "README missing formal_prove docs"
  grep -q "mcpServers" README.md || fail 6 "README missing client configuration snippets"
  pass 6 "Documentation complete with client config snippets"
}
...
case "${GATE}" in
  ...
  "")
    ...
    run_gate_6
    echo ""
    echo -e "\033[0;32m=== All Gates Cleared: mcp-formal Verified ===\033[0m"
    ;;
  *)
